...
class UserProfile():  # ctk.Ctk):
    ...
    def PDFs(self,profile_uploaded_menu_frame):
        ...
        for id, note in path.items():
            ...
            delete_button = CTkButton(master=frame,text='',width=40,height=40,fg_color='#de1313',hover_color='#bd0a0a', command=lambda link=note, nid=id: delete_note('Notes',link,nid,current_user_type))
...
def upload_pdf_using_dialog():
    ...
    if material_type == 'Notes':
        ...
        db.collection("pdfData").add({
            ...
            "uploadedBy": loggedInUser,
            "downloads": 0,
            "link": f"ForApproval/{course}/{branch}/{year}/{subject}/{module}/{current_user_type}"
        })
...
def Approval():
    global admin_logged
    folder_path = f'ForApproval/'
    folder = bucket.list_blobs(prefix=folder_path)
    file_names = [blob.name for blob in folder if blob.name != folder_path]
    print(file_names)       #Directories
    approved=int(input("Do you want to approve?"))
    if approved==1:
        storage.child()
    doc=query1.get()
    data=doc.to_dict()
    ...
    folder_ref = storage_client.bucket().reference(f'ForApproval/')

    # Get all directories in the folder
    directories = folder_ref.list_dirs()
    ...
    if current_user_type == 'Admin':
        if approved==True:
            path=f"Notes/{course}/{branch}/{year}/{subject}/{module}/{current_user_type}/" + file_name'''
